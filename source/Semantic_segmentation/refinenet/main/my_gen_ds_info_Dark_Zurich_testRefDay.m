...
function ds_info = my_gen_ds_info_Dark_Zurich_testRefDay(ds_config)

ds_dir = fullfile('..', '..', '..', '..', 'data', 'Dark_Zurich');

% In Dark Zurich, we obtain the labels as the predictions of a model.
% When this model is first tested on this dataset, all images in the dataset are
% used as validation, since none of them has any annotation yet.
train_full_file_names=cell(0, 1);

val_idx_file = fullfile(ds_dir, 'lists_file_names',...
    'test_ref_filenames.txt');
fid = fopen(val_idx_file);
val_full_file_names = textscan(fid, '%s');
val_full_file_names = val_full_file_names{1};
fclose(fid);

val_full_file_names = fullfile(ds_dir, 'rgb',...
    strcat(val_full_file_names, '_rgb.png'));
