...
function ds_info = my_gen_ds_info_BDD100K_night_trainval_correct(ds_config)

ds_dir = fullfile('..', '..', '..', '..', 'data', 'bdd100k');

% In BDD100K-trainval-night-correct, no image is used for training.
train_full_file_names=cell(0, 1);

val_idx_file = fullfile('..', '..', '..', '..', 'data',...
    'bdd100k_lists_file_names', 'images_trainval_night_correct_filenames.txt');
fid = fopen(val_idx_file);
val_full_file_names = textscan(fid, '%s');
val_full_file_names = val_full_file_names{1};
fclose(fid);

val_full_file_names = fullfile(ds_dir, val_full_file_names);
