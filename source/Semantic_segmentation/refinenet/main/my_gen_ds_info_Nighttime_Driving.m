...
function ds_info = my_gen_ds_info_Nighttime_Driving(ds_config)

ds_dir = fullfile('..', '..', '..', '..', 'data', 'NighttimeDrivingTest');

% In Nighttime Driving, no image is used for training.
train_full_file_names=cell(0, 1);

val_idx_file = fullfile('..', '..', '..', '..', 'data',...
    'NighttimeDrivingTest_lists_file_names', 'leftImg8bit_filenames.txt');
