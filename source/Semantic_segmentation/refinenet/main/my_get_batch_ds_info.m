...
[net_input_info, net_input_str]=gen_net_input_info(opts, imdb, work_info, batch_data, net_input_img_scales);
...
function [net_input_info,net_input_str]=gen_net_input_info(opts, imdb, work_info, batch_data, net_input_img_scales)
...
for s_idx=1:length(net_input_img_scales)
    ...
    % Do not scale also in the case of training when the batch is used for
    % evaluation (which implies that the full image is input to the network).
    % if one_scale==1 || work_info.ref.run_eva
    if one_scale==1 || (~opts.run_evaonly && work_info.ref.run_eva)
